...
class TransacoesController {
  ...
  static async buscarPorIdDoCliente(req, res) {
    console.log(
      "[TRANSACOES CONTROLLER] : CHAMOU O MÉTODO BUSCAR POR ID DO CLIENTE"
    );
    try {
      if (req.params.id == undefined || req.params.id == "") {
        res.status(500).send("Dados de busca insuficientes!");
      } else {
        let objBusca = {};
        let clienteBusca = {};

        clienteBusca._id = req.params.id;
        objBusca.cliente = clienteBusca;

        res.json(await Transacao.find(objBusca));
      }
    } catch (error) {
      console.log("[TRANSACAO CONTROLLER] : buscarTodos => " + error);
      res.status(500).send("Erro ao buscar Transacoes!");
